...
void Pipetadora_AddDispensePoint(int x, int y, int z, int ml) {
    if (dispenseCount < MAX_POINTS) {
        dispensePoints[dispenseCount++] = {x, y, z, ml};
    }
}
...
void Pipetadora_MoveToPosition(int axisId, int target) {
    int current = Pipetadora_GetPositionSteps(axisId);
    if (current < target) {
        if (axisId < MotorCount) Mover_Frente(axisId);
        else stepZForward();
        while (Pipetadora_GetPositionSteps(axisId) < target) {}
        if (axisId < MotorCount) Parar_Mov(axisId);
        else coilsZ = 0;
    } else if (current > target) {
        if (axisId < MotorCount) Mover_Tras(axisId);
        else stepZBackward();
        while (Pipetadora_GetPositionSteps(axisId) > target) {}
        if (axisId < MotorCount) Parar_Mov(axisId);
        else coilsZ = 0;
...
// ----------------------------
// Interpolação XY (Bresenham)
// ----------------------------
#define STEP_DELAY_US 500
static DigitalOut dirXPin(DIR_X), stepXPin(MOTOR_X);
static DigitalOut dirYPin(DIR_Y), stepYPin(MOTOR_Y);

static inline void stepXOnce(int dir) {
    dirXPin = dir > 0;
    stepXPin = 1; wait_us(5);
    stepXPin = 0; wait_us(5);
}

static inline void stepYOnce(int dir) {
    dirYPin = dir > 0;
    stepYPin = 1; wait_us(5);
    stepYPin = 0; wait_us(5);
}

void Pipetadora_MoveInterpolated(int xTarget, int yTarget) {
    int x0 = Pipetadora_GetPositionSteps(0);
    int y0 = Pipetadora_GetPositionSteps(1);
    int dx = abs(xTarget - x0);
    int dy = abs(yTarget - y0);
    int sx = x0 < xTarget ?  1 : -1;
    int sy = y0 < yTarget ?  1 : -1;
    int err = dx - dy;

    while (x0 != xTarget || y0 != yTarget) {
        int e2 = err * 2;
        if (e2 > -dy) {
            stepXOnce(sx);
            x0 += sx;
...
void Pipetadora_ExecutePipetting(void) {
    // Move XY interpolado até ponto de coleta
    ...
    for (int i = 0; i < dispenseCount; i++) {
        ...
        static DigitalOut pipeta(PIPETA);
        pipeta = 0; ThisThread::sleep_for(500ms);
        pipeta = 1; ThisThread::sleep_for(500ms);
    }
}