...
void drawMainMenu(); void drawSubMenu();
void handleMainSelect(); void handleSubSelect();
void telaColeta(); void editaColeta();
void telaSolta();  void editaSolta();
void iniciarPipetagem(); void selecionarML();

void isrUp()    { if(debounceTimer.elapsed_time().count()>debounceTimeMs){debounceTimer.reset();upFlag=true;} }
void isrDown()  { if(debounceTimer.elapsed_time().count()>debounceTimeMs){debounceTimer.reset();downFlag=true;} }
void isrEnter() { if(debounceTimer.elapsed_time().count()>debounceTimeMs){debounceTimer.reset();enterFlag=true;} }
void isrBack()  { if(debounceTimer.elapsed_time().count()>debounceTimeMs){debounceTimer.reset();backFlag=true;} }
void isrEmerg() { emergFlag=true; }

int main() {
    debounceTimer.start();
    Pipetadora_InitMotors();
    buttonUp.fall(&isrUp); buttonDown.fall(&isrDown);
    buttonEnter.fall(&isrEnter); buttonBack.fall(&isrBack);
    buttonEmerg.fall(&isrEmerg);
...
void drawMainMenu() {
    ...
    for(int i=0;i<MAIN_COUNT;i++){ lcd.locate(1,i+1); lcd.printf("%s",mainMenu[i]); }
    cursor=0; inSubmenu=false; lcd.locate(0,1); lcd.putc('>');
}

void drawSubMenu() {
    lcd.cls(); lcd.locate(3,0); lcd.printf("PIPETADORA");
    for(int i=0;i<SUB_COUNT;i++){ lcd.locate(1,i+1); lcd.printf("%s",subMenu[i]); }
    cursor=0; inSubmenu=true; lcd.locate(0,1); lcd.putc('>');
}
...
void handleSubSelect() {
    ...
    if(backFlag) { drawMainMenu(); backFlag=false; }
}

void telaColeta() {
    int x,y,z; Pipetadora_GetCollectionPoint(&x,&y,&z);
    ...
    if(backFlag)  { drawSubMenu(); backFlag=false; }
    enterFlag=false;
}
...
void editaColeta() {
    ...
    lcd.cls(); lcd.printf("Coleta salvo"); thread_sleep_for(500);
    drawSubMenu(); enterFlag=false;
}
...
void editaSolta() {
    int count = Pipetadora_GetDispenseCount();
    lcd.cls(); lcd.printf("Ponto #%d: Mover", count+1);
    while(!enterFlag) { Pipetadora_ManualControl(); }
    int sx = Pipetadora_GetPositionSteps(0);
    int sy = Pipetadora_GetPositionSteps(1);
    int sz = Pipetadora_GetPositionSteps(2);
    lcd.cls(); lcd.printf("Sel ML"); selecionarML();
